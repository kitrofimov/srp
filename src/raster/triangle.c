...
static bool shouldCullTriangle(const SRPTriangle* tri, bool* isCCW, bool* isFrontFacing)
{
	...
	*isCCW = signedArea > 0;
	...
	bool frontFacing = \
		(signedArea > 0) & (srpContext.raster.frontFace == SRP_FRONT_FACE_CCW) ||
		(signedArea < 0) & (srpContext.raster.frontFace == SRP_FRONT_FACE_CW);
	bool cull = \
		( frontFacing && srpContext.raster.cullFace == SRP_CULL_FACE_FRONT) ||
		(!frontFacing && srpContext.raster.cullFace == SRP_CULL_FACE_BACK);
